...
export default function StudentAssignmentList() {
  ...
  const fetchAssignables = async () => {
    try {
      setLoading(true);
      console.log('Fetching assignables...');
      console.log('Token:', localStorage.getItem('token')); // Log token for debugging
      
      const response = await axios.get('/assignables/', {  // Added trailing slash
        headers: {
      ...
      console.error('Error fetching assignables:', {
        message: err.message,
        response: err.response?.data,
        status: err.response?.status,
        headers: err.response?.headers
      });
      setError(`Failed to fetch assignables: ${err.response?.data?.detail || err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>, assignableId: number) => {
    ...
    try {
      ...
      const response = await axios.post(
        `/assignables/${assignableId}/submit`,
        formData,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const handleSubmissionDownload = async (assignmentId: number, fileName: string) => {
    try {
  ...
  const renderAssignableType = (doc: AssignableType, index: number) => {
          ...
          <Grid container spacing={2}>
            {ongoingAssignables.map((doc, index) => (
              <Grid item xs={12} key={doc.id}>
                {renderAssignableType(doc, index)}
              </Grid>
            ))}
