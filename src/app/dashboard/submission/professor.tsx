...
export default function ProfessorSubmissionList() {
  ...
  const handleUpdateSubmittable = (submittableId: number) => {
    router.push(`/dashboard/submission/update?id=${submittableId}`);
  };

  const handleCreateSubmittable = () => {
    router.push('/dashboard/submission/creation');
  };

  const handleViewSubmissions = (submittableId: number) => {
    router.push(`/dashboard/submission/view?id=${submittableId}`);
  };
